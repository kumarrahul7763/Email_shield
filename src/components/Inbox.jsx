...
const Inbox = ({ onClassify, onError }) => {
  ...
  const getCategoryHint = (subject) => {
    ...
    if (lowerSubject.includes('offer') || lowerSubject.includes('free') || lowerSubject.includes('partner') || lowerSubject.includes('promotion') || lowerSubject.includes('discount')) {
  ...
  return (
    <div className="card-gradient animate-fade-in">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h2 className="text-2xl font-bold text-gray-200">Inbox</h2>
          <p className="text-gray-400 mt-1">Click an email to auto-classify it</p>
        </div>
      </div>
          ...
          <div
            ...
            className={`
              ...
              ${selectedEmail === email.id 
                ? 'border-purple-500 bg-gradient-to-r from-purple-500/10 to-pink-500/10 shadow-lg shadow-purple-500/20 scale-[1.02]' 
                : 'border-white/10 hover:border-purple-500/50 hover:bg-gradient-to-r hover:from-purple-500/5 hover:to-pink-500/5 hover:shadow-md'
              }
              ${!email.read ? 'bg-gradient-to-r from-blue-500/5 to-indigo-500/5' : ''}
            `}
          >
            {/* Background decoration */}
            <div className="absolute inset-0 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              <div className="absolute -top-10 -right-10 w-32 h-32 bg-gradient-to-br from-purple-500/10 to-pink-500/10 rounded-full blur-2xl"></div>
            </div>
            ...
            <div className="relative z-10 flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3 mb-2">
                  {!email.read && (
                    <span className="w-2.5 h-2.5 bg-gradient-to-r from-purple-500 to-pink-500 rounded-full animate-pulse"></span>
                  )}
                ...
                <h3 className="text-base font-bold text-gray-200 mb-2 truncate">
                  {email.subject}
                </h3>
              ...
              <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold ${getCategoryHint(email.subject)}`}>
                ...
                <span>
                  {email.subject.toLowerCase().includes('invoice') ? 'Finance' :
                   ...
                   email.subject.toLowerCase().includes('winner') ? 'Spam' :
                   email.subject.toLowerCase().includes('offer') || email.subject.toLowerCase().includes('free') || email.subject.toLowerCase().includes('partner') || email.subject.toLowerCase().includes('promotion') ? 'Promotional / Marketing' : 'Support'}
                </span>
              </div>
            </div>
            ...
            {selectedEmail === email.id && isClassifying && (
              <div className="relative z-10 mt-4 flex items-center gap-3 text-purple-400">
      ...
      {/* Info box */}
      <div className="mt-8 p-5 bg-gradient-to-r from-violet-500/10 to-purple-500/10 rounded-2xl border border-violet-500/20 backdrop-blur-sm">
